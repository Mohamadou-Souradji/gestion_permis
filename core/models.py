...
class JournalImpression(models.Model):
    """Enregistrement simple de chaque impression"""
    agent = models.ForeignKey(ProfilAgent, on_delete=models.SET_NULL, null=True, blank=True)
    utilisateur = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    type_fiche = models.CharField(max_length=50)
    candidat = models.ForeignKey(Candidat, on_delete=models.SET_NULL, null=True, blank=True)
    date_impression = models.DateTimeField(auto_now_add=True)
    adresse_ip = models.CharField(max_length=50, blank=True)
    
    class Meta:
        ordering = ['-date_impression']
        verbose_name = "Journal impression"
    
    def __str__(self):
        return f"{self.date_impression.strftime('%d/%m %H:%M')} - {self.type_fiche} - {self.candidat}"
