...
from .models import (
    ...
    JournalAudit, CategoriePermis, ParametreImpression, JournalImpression,
...
def imprimer_attestation(request, pk):
    ...
    positions = list(ParametreImpression.objects.filter(type_fiche='attestation', visible=True))
    for p in positions:
        p.x_px = round(p.x_mm * MM_TO_PX)
        p.y_px = round(p.y_mm * MM_TO_PX)
    ...
    enregistrer_impression(request, 'attestation', candidat)
...
def test_impression(request, type_fiche):
    ...
    for p in positions:
        p.x_px = round(p.x_mm * MM_TO_PX)
        p.y_px = round(p.y_mm * MM_TO_PX)
        positions_avec_px.append(p)
    
    return render(request, 'core/admin/test_impression.html', {
        'type_fiche': type_fiche,
        'positions': positions_avec_px,
        'label': dict(ParametreImpression.TYPE_FICHE_CHOICES).get(type_fiche, type_fiche),
    })
...
def imprimer_fiche_controle(request, pk):
    candidat = get_object_or_404(Candidat, pk=pk)
    positions = list(ParametreImpression.objects.filter(type_fiche='fiche_controle', visible=True))
    ...
    enregistrer_impression(request, 'fiche_controle', candidat)
...
def fiche_examen_code(request):
    ...
    examen = None
    valeurs = {}
    positions = list(ParametreImpression.objects.filter(type_fiche='fiche_examen_code', visible=True))
    ...
    if numero:
        ...
        if candidat:
            ...
            enregistrer_impression(request, 'fiche_examen_code', candidat)
...
def fiche_examen_conduite(request):
    ...
    examen = None
    valeurs = {}
    positions = list(ParametreImpression.objects.filter(type_fiche='fiche_examen_conduite', visible=True))
    ...
    if numero:
        ...
        if candidat:
            ...
            valeurs = _valeurs_fiche_examen(candidat, examen, date.today())
            enregistrer_impression(request, 'fiche_examen_conduite', candidat)
...
def enregistrer_impression(request, type_fiche, candidat=None):
    """Enregistre une impression dans le journal"""
    try:
        agent = None
        ...
        JournalImpression.objects.create(
            agent=agent,
